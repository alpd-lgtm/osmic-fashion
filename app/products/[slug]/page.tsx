...
export default async function ProductDetailPage({
  ...
  return (
    <main className="mx-auto max-w-7xl bg-white px-4 pt-4 pb-8 sm:px-6 sm:pt-6 lg:px-8">
      ...
      {/* Product Detail */}
      <section className="rounded-[2rem] bg-[#FFFDFC] p-4 ring-1 ring-gray-100 sm:p-6 lg:p-8">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12">
          {/* Image */}
          <div className="relative">
            <div className="relative h-[320px] w-full overflow-hidden rounded-[2rem] bg-[#FAF7F3] ring-1 ring-gray-100 sm:h-[420px] lg:h-[520px]">
              <Image
                src={product.image}
                alt={product.name}
                fill
          ...
          <div className="flex flex-col justify-center">
            ...
            <h1 className="mt-3 text-2xl font-bold leading-tight text-[#111111] sm:text-3xl lg:text-4xl">
              {product.name}
            </h1>

            <p className="mt-3 text-xl font-semibold text-[#111111] sm:text-2xl">
              NPR {product.price}
            </p>

            <div className="mt-4 h-px w-16 bg-[#7A1F2A]" />
      ...
      {relatedProducts.length > 0 && (
        <section className="mt-10 rounded-[2rem] bg-[#FCFAF8] px-4 py-8 sm:mt-12 sm:px-6 sm:py-9">
          <div className="mb-6">
            <p className="text-xs uppercase tracking-[0.28em] text-[#7A1F2A]">
              You may also like
            </p>
